 ...
 * @private
 */
const { constants, defaults: defaultOpts } = require('@microfleet/config');
...
class Mservice extends EventEmitter {
  ...
  static routerExtension(name: string) {
    ...
    return require(`@microfleet/plugin-router/src/extensions/${name}`);
  }
  ...
  _initPlugins(config: Object) {
    ...
    config.plugins.forEach((plugin) => {
      // eslint-disable-next-line import/no-dynamic-require
      this.initPlugin(require(`@microfleet/plugin-${plugin}`));
    });
