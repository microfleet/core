...
describe('Router suite', function testSuite() {
  ...
  const auditLog = require('../src/extensions/audit/log');
  const getAMQPRequest = require('./helpers/requests/amqp');
  const getHTTPRequest = require('./helpers/requests/http');
  const getSocketIORequest = require('./helpers/requests/socketIO');
  const verify = require('./helpers/verifyCase');
  ...
  it('should return response', function test() {
    const service = new Mservice({
      ...
      plugins: ['validator', 'logger', 'router', 'amqp', 'http', 'socket-io'],
      router: {
        routes: {
          directory: path.resolve(__dirname, './helpers/actions'),
          enabled: {
            simple: 'simple',
            retry: 'retry',
          },
      ...
      validator: [path.resolve(__dirname, './helpers/schemas')],
    });
