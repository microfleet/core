...
import { KafkaFactory, KafkaConsumerStream, KafkaProducerStream } from '@microfleet/plugin-kafka'
...
declare module 'node-rdkafka' {
  ...
  export interface ConsumerStream {
    new (consumer: KafkaConsumer, conf: ConsumerStreamOptions): ConsumerStream;
  ...
  export interface Client extends NodeJS.EventEmitter {
    ...
    // add .code prop
    on(event: 'rebalance', listener: (err: KafkaError, assignment: any) => void): this;
  }
  ...
  export interface ConsumerGlobalConfig extends GlobalConfig {
     ...
     */
    'rebalance_cb'?: boolean | ((error: Error | undefined, assignments: TopicPartition[]) => void);

    /**
     * Offset commit result propagation callback. (set with rd_kafka_conf_set_offset_commit_cb())
     */
    'offset_commit_cb'?: boolean | ((error: Error | undefined, topicPartitions: TopicPartition[]) => void);
