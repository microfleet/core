...
class Mservice extends EventEmitter {
  ...
  exit() {
    ...
    return this.close().timeout(1000).finally(() => {
      process.exit(0);
    });
  }
  ...
  hook(event, ...args) {
    ...
    return Promise.bind(this).return(listeners).map(function performOp(listener) {
  ...
  _processAndEmit(arr, event) {
    return Promise.map(arr, func => func()).tap(() => {
      this.emit(event);
    });
  }
