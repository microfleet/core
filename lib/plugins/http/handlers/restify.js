...
function createRestifyServer(config, service) {
  ...
  function startServer() {
    ...
    return Promise.fromCallback(callback => service.http.server.listen(config.server.port, config.server.host, callback)).then(() => service.emit('plugin:start:http', service.http)).then(() => service.http);
  }
  ...
  function stopServer() {
    ...
    return Promise.fromCallback(next => service.http.close(next)).timeout(5000).catch(Promise.TimeoutError, () => Promise.fromCallback(next => service.http.destroy(next))).then(() => service.emit('plugin:stop:http'));
  }
