...
function dispatch(route, request, callback) {
  ...
  const result = Promise.resolve([route, request]).bind(router.service).spread(router.modules.request).then(router.modules.auth).then(router.modules.validate).then(router.modules.allowed).then(router.modules.handler).tap(() => {
