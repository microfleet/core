...
describe('router: module lifecycle', function suite() {
  it('should reject "module" argument error', function test(done) {
    ...
    moduleLifecycle({}, () => {}, extensions, []).reflect().then(inspection => {
  ...
  it('should reject "promiseFactory" argument error', function test(done) {
    ...
    moduleLifecycle('foo', 'promiseFactory', extensions, []).reflect().then(inspection => {
      const error = inspection.reason();
      expect(error).to.be.instanceof(Errors.ArgumentError);
  ...
  it('should reject "extensions" argument error', function test(done) {
    moduleLifecycle('foo', () => {}, [], []).reflect().then(inspection => {
      const error = inspection.reason();
      expect(error).to.be.instanceof(Errors.ArgumentError);
  ...
  it('should reject "args" argument error', function test(done) {
    ...
    moduleLifecycle('foo', () => {}, extensions, '').reflect().then(inspection => {
      const error = inspection.reason();
      expect(error).to.be.instanceof(Errors.ArgumentError);
  ...
  it('should return error from "pre-handler"', function test(done) {
    const extensions = new Extensions({
      enabled: ['preFoo'],
      register: [[{ point: 'preFoo', handler: (foo, bar) => Promise.resolve([foo, bar]) }, { point: 'preFoo', handler: (foo, bar) => Promise.reject(`error: ${ bar }`) }]]
    });
    const handler = (foo, bar) => Promise.resolve(`result: ${ bar }`);

    moduleLifecycle('foo', handler, extensions, ['foo', 'bar']).reflect().then(inspection => {
      const error = inspection.reason();
  ...
  it('should return result from handler with "pre-handler"', function test(done) {
    const extensions = new Extensions({
      enabled: ['preFoo'],
      register: [[{ point: 'preFoo', handler: (foo, bar) => Promise.resolve([bar, 'baz']) }]]
    });
    const handler = (bar, baz) => Promise.resolve(`result: ${ baz }`);

    moduleLifecycle('foo', handler, extensions, ['bar', 'foo']).reflect().then(inspection => {
  ...
  it('should return result from handler with "pre-handler" that takes one argument', done => {
    const extensions = new Extensions({
      enabled: ['preFoo'],
      register: [[{ point: 'preFoo', handler: request => Promise.resolve(`${ request } bar`) }, { point: 'preFoo', handler: request => Promise.resolve(`${ request } baz`) }]]
    });
    const handler = request => Promise.resolve(`result: ${ request }`);

    moduleLifecycle('foo', handler, extensions, ['foo']).reflect().then(inspection => {
  ...
  it('should return error from post-handler', function test(done) {
    const extensions = new Extensions({
      enabled: ['postFoo'],
      register: [[{ point: 'postFoo', handler: (error, result) => Promise.resolve([error, result]) }, { point: 'postFoo', handler: (error, result) => Promise.reject(`error: ${ result }`) }]]
    });
  ...
  it('should be able to modify error returned from handler', function test(done) {
    const extensions = new Extensions({
      enabled: ['postFoo'],
      register: [[{ point: 'postFoo', handler: (error, result) => Promise.resolve(['baz', result]) }]]
    });
  ...
  it('should be able to pass arguments to post-handler', function test(done) {
    const extensions = new Extensions({
      enabled: ['postFoo'],
      register: [[{ point: 'postFoo', handler: (error, result, foo, bar) => Promise.resolve([foo + bar]) }]]
    });
    const handler = (foo, bar) => Promise.resolve(`${ foo }.${ bar }`);
