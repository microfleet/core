...
function moduleLifecycle(module, promiseFactory, extensions, args, context) {
  ...
  if (extensions.has(preModule)) {
    result = Promise.resolve([preModule, args, context]).bind(extensions).spread(extensions.exec);
  } else {
    result = Promise.resolve(args);
  }

  return result.bind(context).spread(promiseFactory).reflect().then(inspection => {
    ...
    return Promise.resolve([postModule, [errorResponse, resultResponse].concat(_toConsumableArray(args)), context]).bind(extensions).spread(extensions.exec).spread((error, response) => {
      if (error) {
