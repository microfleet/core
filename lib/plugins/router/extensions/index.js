...
class Extensions {
  ...
  exec(name, args = [], context = null) {
    ...
    return Promise.resolve(handlers).reduce((previousArgs, handler) => convertToArrayIfNot(previousArgs).bind(context).spread(handler), args).then(convertToArrayIfNot);
  }
}
