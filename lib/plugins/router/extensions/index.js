...
class Extensions {
  ...
  exec(name, args = [], context = null) {
    ...
    return Promise.resolve(handlers).bind(context).reduce((previousArgs, handler) => convertToArrayIfNot(previousArgs).spread(handler), args).then(convertToArrayIfNot);
  }
}
